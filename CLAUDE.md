...
- **`?status=` is the list's third filter**, URL-state like search; an unrecognised value degrades to "no filter" **at both layers** — `statusFilter()` in `clients/page.tsx` before the query is built, and `ClientValidation.StatusFilter` in the API, so a direct caller (curl, an MCP tool) gets the same answer as the screen rather than an empty list that looks like an empty database. `?industry=` gets no equivalent: it is free text with no closed set to check. Statuses/kinds are CHECK-constrained lists mirrored in three places: migration `0002`, `ClientValidation.ClientStatuses`/`InteractionKinds`, and `CLIENT_STATUSES`/`INTERACTION_KINDS` in `types.ts` — change all three together.
