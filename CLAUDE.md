...
- **`owner` is a raw Clerk sub** — there is no user directory, so the UI offers "Assign to me"/"Unassign" and renders "you" for the signed-in user's own id (`useUser()` client-side, display only). Because PATCH is full-replacement, `updateClientFromRow` now read-first-preserves `notes` **and `source` and `owner`**, and the detail edit form passes `owner` through from props.
