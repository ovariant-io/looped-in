...
`/clients` is CRUD over a shared client list and its contacts — the first real SQL schema in the repo (`clients` + `contacts`, migration `0001`). Migration `0002` turned the list into a **pipeline**: lifecycle columns on `clients` (`status`, `acquired_at`, `source`, `owner`, `lost_reason`), an append-only `client_status_history` audit table, and an `interactions` table (the per-client outreach log, with an optional `contact_id` that is `on delete set null` so pruning a contact never erases the record that a touch happened).

- **Status moves only through `POST /clients/{id}/status`, never through PATCH.** A transition is an event: the endpoint appends the history row atomically with the update (one data-modifying CTE chain in `ChangeStatusAsync` — the unreferenced `recorded` CTE still executes, it is not dead code), stamps `acquired_at` on the first move to `active_client`, and clears `lost_reason` on any move away from `lost`. That split is also what makes the cross-column `clients_lost_reason_shape` CHECK safe — PATCH (which now carries `source` + `owner`) cannot touch either column. Same-status transitions are allowed: `lost → lost` is how a lost reason gets corrected. History and the interaction log are read via `GET /clients/{id}/status-history` and `/clients/{id}/interactions` (404 for a missing client, not an empty list); interactions are CRUD like contacts, including `expectedVersion` on PATCH.
- **`owner` is a raw Clerk sub** — there is no user directory, so the UI offers "Assign to me"/"Unassign" and renders "you" for the signed-in user's own id (`useUser()` client-side, display only). Because PATCH is full-replacement, `updateClientFromRow` now read-first-preserves `notes` **and `source`, `owner`, `website` and `whatTheyDo`**, and the detail edit form passes `owner` through from props.
- **`website` and `whatTheyDo` are descriptive fields** (migration `0003`), edited through PATCH like the rest and absent from `ClientSummary` — the two prose fields for the same reason `notes` is, `website` because the list table is already seven columns wide. They are **not** in the list's search predicate either; adding them means `ListFilter` in `ClientStore` **and** the search box's placeholder text.
- **`website` is the one field the API rewrites.** A scheme-less `looped-in.com.au` is stored as `https://looped-in.com.au`, because the detail page renders the stored value straight into an `href` and a scheme-less string there is a *relative path*. Normalizing at the boundary is also what makes the scheme allow-list in `TryReadWebsite` a real control: `javascript:`/`data:` URLs are refused at the one place every write passes through, rather than filtered at each render site where the next new one forgets. **URLs carrying userinfo are refused too** — `@` is what lets `mailto:someone@example.com` parse as a valid https URL for host `example.com` (username `mailto:someone`), and on a list everyone can edit, `https://looped-in.com.au@evil.com` is a link that reads as the client's own domain. **Non-ASCII hosts are refused for the same reason**: browsers apply IDNA before resolving, and U+3002/U+FF0E/U+FF61 all normalize to a label separator, so `looped-in.com.au。evil.com` reads as the client's domain and resolves as a subdomain of `evil.com` (`Uri.Host` vs `Uri.IdnHost` is exactly that divergence) — a punycode `xn--` host is still accepted, since it is what resolves anyway. Unlike `IsPlausibleEmail` these rules have no importer to stay equivalent to: `import_clients.py` names its insert columns explicitly and never writes either new column.
- **What the website rules do and don't guarantee.** A stored value is an absolute `http(s)` URL whose visible text names the host a browser will dial — that is all. It is **not** a promise the host is who it looks like: `looped-in.com.au.evil.com` is a subdomain of `evil.com`, reads like the client's own domain, and nothing short of a domain allow-list refuses it. Both rules above defend the narrow guarantee; treat the field as attacker-controlled text that is safe *in an `href`*, not as a verified identity.
- **The website field's `maxLength={500}` mirrors the column, not the effective limit.** The API measures the value *after* `https://` is prepended, so a scheme-less entry really stops at 492. Capping the input at 492 would reject a legitimate 500-character URL that carries its own scheme, so the field stays permissive and `TryReadWebsite` names both lengths in the error rather than reporting a count the user didn't type.
- **`?status=` is the list's third filter**, URL-state like search; an unrecognised value degrades to "no filter" **at both layers** — `statusFilter()` in `clients/page.tsx` before the query is built, and `ClientValidation.StatusFilter` in the API, so a direct caller (curl) gets the same answer as the screen rather than an empty list that looks like an empty database. The MCP `list_clients` tool takes the opposite stance on purpose: its `status` is a schema `Literal`, because an agent would read silently-unfiltered results as the filtered answer. `?industry=` gets no equivalent: it is free text with no closed set to check. Statuses are a CHECK-constrained list mirrored in **four** places: migration `0002`, `ClientValidation.ClientStatuses`, `CLIENT_STATUSES` in `types.ts`, and `ClientStatus` in `mcp/looped_in_mcp/tools/clients.py`; interaction kinds in the same four (`InteractionKind` on the MCP side, since `add_client_interaction` takes a kind) — change every mirror together.
- **The status control is keyed on `client.status`** (`StatusForm` in `clients/[id]/client-detail.tsx`). It has to be: the version it submits is read from live props, so if a stale selection could outlive a transition by someone else, the pair would still validate and silently revert them. Remounting on the status is what keeps the submitted status and version describing one row — the contact and interaction editors get the same guarantee from their `EditTarget` version snapshot instead, because their inputs are uncontrolled.
- **A client's contacts and interactions are read whole, unpaged**, unlike the client list. Recorded in `ListInteractionsAsync`: the page shows them whole, and paging both is the fix when a real client's history stops fitting a screen — the same deferral shape as `pg_trgm` for search.
- **`GET /clients/details` is the bulk read** — the same page `/clients` would select, but each row in full (`{ client, lastInteraction }`: the `GET /clients/{id}` shape with contacts, plus the newest log entry), under the same filters and paging. It exists to kill the N+1 for callers that personalize or recency-check across the whole list — over MCP, a 154-client campaign draft was one `get_client` (and one tool approval) per row before this. The row composes `ClientDetail` rather than flattening it, so a field added to the detail can't be forgotten in the bulk shape; no screen consumes it — the list page's seven columns are already the deliberate UI trim.
- **The MCP server exposes this data as full CRUD** (`mcp/looped_in_mcp/tools/clients.py`): the five readers (`list_clients`, `list_client_details`, `get_client`, `get_client_status_history`, `list_client_interactions`) plus ten writers covering clients, status transitions, contacts, and interactions — thin wrappers over the `/clients` routes that forward the caller's Clerk token, so the API's auth, validation and 503 gating do all the work. The agent-facing write policy lives in the tool layer: every update requires `expected_version` from a fresh read, and because the API's PATCH is full-replacement (null clears), the update tools **merge** — they re-read the row, preserve unmentioned fields, and null only fields named in `clear`, while the wire request still carries the caller's `expected_version` so concurrent edits 409 rather than overwrite. Deletes are annotated destructive and their docstrings direct the agent to confirm with the user. Three surfaces describe this tool set and must move together: `mcp/README.md`, the `/connect` guide's `TOOLS` list, and `ASSISTANT_PROMPT` in `frontend/app/(app)/connect/assistant-prompt.ts`.
...
## Campaigns (EDM drafting)

`/campaigns` is EDM (email marketing) **drafting**, deliberately without sending: a `campaigns` table (name + drafting brief) and `campaign_messages` (one drafted email **per client per campaign** — unique `(campaign_id, client_id)`, and `DatabaseGateFilter` names that constraint for a specific 409), migration `0004`. Approved messages are copied/exported into the team's mail tool; a send is a human act the API records.

- **Messages carry the state machine (`drafted / approved / sent / skipped`); campaigns have no status column.** A campaign's progress is the derived per-state counts in `CampaignStore.ListAsync` — which makes that query, `CampaignSummary`, and the list screen's chips de-facto mirrors of the vocabulary alongside migration `0004`, `CampaignValidation.CampaignMessageStates`, `CAMPAIGN_MESSAGE_STATES` in `campaigns/types.ts`, and `CampaignMessageState` in `mcp/looped_in_mcp/tools/campaigns.py`. **Five mirror sites — change them together.**
- **State moves only through `POST /campaigns/{id}/messages/{mid}/state`, never PATCH** — the `ChangeStatusAsync` shape applied across tables. Transitions are free (any→any, same-state legal), but entering `sent` stamps `sent_at` and **atomically appends an `email` interaction to the client's outreach log** — there is deliberately no separate state-history table; that interaction *is* the audit of a send, and it stays ordinary CRUD so a mis-recorded send can be retracted (move off `sent`, which clears `sent_at`, then delete the interaction). The append is gated on the *pre-update* state, so `sent → sent` never double-logs; agents are told the state change logs the touch for them (the double-logging hazard lives in `ASSISTANT_PROMPT`'s campaign section).
- **Bodies are plain text** (paragraphs = blank lines, limit 10000 — deliberately past the notes tier because the body is the artifact). The rendered email is built by `app/lib/email-template.ts`: 100% first-party markup with every interpolation escaped, which is what makes the detail page's sandboxed `<iframe srcDoc>` preview safe without a sanitizer dependency. **Email colours are baked from `DEFAULT_PALETTE` at module load** — email clients support neither `var()` nor `color-mix`, and an outbound email is brand collateral, not a per-browser preference — so the colour picker restyles the app but never the rendered email. The template must stay an **imported TS module**, never an fs-read asset (`output: "standalone"` tracing).
- **Frontend `app/(app)/campaigns/`** follows `clients/` exactly (URL-state list, server actions, `EditTarget` snapshots, a `StateForm` keyed on `message.state` like `StatusForm`). The detail page's add-message client picker is server-fetched at `/clients?limit=200` — the API's `MaxPageSize`, so it silently truncates past 200 clients (recorded deferral; the seeded list is already at ~191). The recipient is set on the message card, not the add form: recipient options ride `CampaignDetail.contactOptions`, since client list summaries don't carry contacts. `do_not_contact` clients are disabled in the picker — UI-level suppression; the API deliberately doesn't enforce it.
- **The MCP server exposes campaigns as 9 tools** (`tools/campaigns.py`): `list_campaigns` / `get_campaign` readers plus campaign CRUD, message CRUD and `set_campaign_message_state` — same merge/`expected_version`/confirm-deletes policy as the client tools. The same three surfaces as ever must move with the tool set: `mcp/README.md`, the `/connect` `TOOLS` list, and `ASSISTANT_PROMPT` (which now carries the drafting doctrine: suppression, personalize-only-from-populated-fields, drafts-into-campaigns-not-chat, sending-is-human).
- **Deleting a client now cascades to its campaign drafts** (and deleting a contact nulls the drafts' recipient — the UI renders "no recipient"). `GET /campaigns/{id}` is unpaged with full bodies, a recorded deferral like the interactions log.
...
**`test` is deployed and live; `prod` has never been deployed.** `sst.config.ts` is a thin entry point that dynamically imports the resource modules in **`infra/`** (SST forbids top-level imports there). `infra/index.ts` composes the graph; `infra/README.md` documents ownership per module, extension recipes, and the change checklist. **Read it before editing anything under `infra/`.**

- **Check stage state rather than trust this file.** **`npx sst state list`** is the authoritative answer to "what exists" — it names every deployed stage (today: `test`; `prabath` is in state but *not deployed*). There is **no `sst outputs` command**: the `web` / `api` / `mcp` / `bucket` values are printed by the deploy itself and cached in the gitignored `.sst/outputs.json` for whichever stage *this machine* deployed last — so that file is not evidence about any other stage. The generated CloudFront and API Gateway ids are deliberately **not** written down here: they change whenever a stage is removed and recreated, and a hardcoded URL is exactly how this section's previous "nothing has been deployed yet" became false. A healthy stage answers `/db/ping`, `/auth/ping` and `/documents/ping` on the API origin, and `/health` plus `/.well-known/oauth-protected-resource/mcp` on the MCP one. **`prod` has no resources at all** — `assertStageDeployable` blocks it while `STAGE_CONFIG.prod.corsAllowOrigins` is `[]`.
- **A deployed `test` makes the Clerk sign-up gate a live concern, not a future one.** `/clients` rows are shared by every signed-in user with `.RequireAuthorization()` as the only defence (see **Clients**), so restrict who can sign up to the Clerk instance before this stage holds real client data.
