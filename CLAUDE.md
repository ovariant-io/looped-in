...
- **Semantic tokens flip with the colour scheme; raw `--li-*` colours don't.** Dark mode uses a warm near-black in the eggplant hue (`--li-night`, `#1a130a`) rather than a neutral inversion, and swaps `--li-meta`/`--li-rule` to sky — indigo goes muddy on it, sky is illegible on cream. Other tokens: `--li-surface`, `--li-line`, `--li-accent-foreground`, `--li-radius-button` (6.4px), `--li-radius-panel`, `--li-heading-tracking`, `--li-heading-leading`, `--li-body-leading`.
- **There are seven raw colours, not five.** `--li-night` (the dark ground) and `--li-plum` (`#351f40`, the wordmark ink and the scene's client nodes) join the five brand values above. Both were literals before — `#1a130a` inside the dark media block, `0x351f40` inside `loop-scene.tsx` — and are tokens now so the colour picker can drive them.
- **The dark scheme is declared twice, and must stay identical.** `@media (prefers-color-scheme: dark) :root:not([data-li-scheme="light"])` and `:root[data-li-scheme="dark"]`. CSS cannot OR a media query with a selector, and the picker has to be able to *pin* a scheme so the dark half of a palette can be judged from a light-mode machine.
...
- **The logo is the real artwork, in `public/`.** `looped-in-logo.png` is the lockup the brand site serves (plum `#351f40` wordmark + indigo `#4f67b1` mark, transparent); `looped-in-logo-dark.png` is the same file with the wordmark re-inked to cream, because plum on the dark ground is ~1.4:1. `app/lib/brand-logo.tsx` picks between them with a `<picture>` media source — no JS, and only the matching file is fetched. **Note the wordmark ink is a plum, not the `#472700` the token table calls eggplant** — the Squarespace theme value and the logo artwork genuinely differ, so `--li-eggplant` was left alone rather than redefined. A `<picture>` source cannot see the picker's scheme pin, so `brand-logo.module.css` paints the correct artwork as a `background-image` when — and only when — `[data-li-scheme]` is set; an unmatched rule is never requested, so the unpinned single-fetch path is untouched.
...
- **The landing scene reads `--li-scene-mark`, `--li-scene-node` and `--li-rule`** instead of holding hexes, and re-reads on the `PALETTE_EVENT` (`looped-in:palette`) the picker dispatches. That is what makes a WebGL material and a CSS border the same colour by construction.
...
- **`app/page.tsx` + `app/loop-scene.tsx` are the reference implementation.** The landing is one screen: lockup, headline, two CTAs, and a `three` scene of the brand mark ringed by AI clients. `three` is **dynamically imported** so ~600 KB stays off first paint; the scene labels are **projected HTML, not sprites**, so they are real text that themes from CSS and stays readable when WebGL is blocked (they sit in normal flow until the scene sets `data-projected`). The scene swaps palette with the colour scheme for the same reason `--li-rule` does: sky is ~1.03:1 on cream and indigo goes muddy on the dark ground.
