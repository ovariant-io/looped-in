...
- **The MCP server exposes this data read-only** (`mcp/looped_in_mcp/tools/clients.py`): `list_clients`, `get_client`, `get_client_status_history`, `list_client_interactions` — thin wrappers over the GET routes that forward the caller's Clerk token, so the API's auth and 503 gating do all the work. Deliberately no write tools yet: mutations need the `expectedVersion` flow and a considered policy for agents changing a shared list.
