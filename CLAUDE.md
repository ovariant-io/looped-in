...
- **`?status=` is the list's third filter**, URL-state like search; a bad value degrades to "no filter". Statuses/kinds are CHECK-constrained lists mirrored in three places: migration `0002`, `ClientValidation.ClientStatuses`/`InteractionKinds`, and `CLIENT_STATUSES`/`INTERACTION_KINDS` in `types.ts` — change all three together.
